...
kmRuntimeUse(0x809c2108); //USERHandler::sInstance

static u8 REGIONID = 0x0A;

static void SetRegionId() {
    if (Pulsar::System::sInstance->IsContext(PULSAR_STARTCTS))
        REGIONID = 0x14;
    else
        REGIONID = 0x0A;
}
...
static bool ConvertFriendRoomStateToRegional() {
    ...
    //USERHandler is fully private in this GameSource, so reach the instance and its isInitialized flag by address
    RKNet::USERHandler** userHandler = reinterpret_cast<RKNet::USERHandler**>(kmRuntimeAddr(0x809c2108));
    if (*userHandler != nullptr) {
        *reinterpret_cast<u8*>(*userHandler) = 0; //isInitialized is the first data member
    }
    ...
    //context is private here, but UpdateContext overwrites it wholesale so recomputing is enough
...
static void SetNextSectionRegionalHook(SectionMgr *sectionMgr, SectionId nextSectionId, u32 animDirection) {
    ...
    if (Pulsar::System::sInstance->IsContext(PULSAR_STARTCTS) && isFroom) {
